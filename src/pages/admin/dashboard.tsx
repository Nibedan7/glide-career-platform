...
import { Users, Briefcase, UserCheck, TrendingUp, Eye, Check, X, Ban } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const allUsers = [
    ...students.map(s => ({ ...s, role: "student" as const, status: s.id === "1" ? "pending" as const : "approved" as const })),
    ...companies.map(c => ({ ...c, role: "company" as const, email: `contact@${c.name.toLowerCase().replace(/\s+/g, '')}.com`, avatar: c.logo, joinedAt: "2024-01-10T00:00:00Z" })),
    ...mentors.map(m => ({ ...m, role: "mentor" as const, status: m.id === "1" ? "pending" as const : "approved" as const, joinedAt: "2024-01-12T00:00:00Z", email: `${m.name.toLowerCase().replace(/\s+/g, '.')}@email.com` }))
  ];

  // Filter users by status and role
  const pendingUsers = allUsers.filter(user => user.status === "pending");
  const pendingStudents = pendingUsers.filter(user => user.role === "student");
  const pendingCompanies = pendingUsers.filter(user => user.role === "company");
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Manage users, monitor platform activity, and oversee operations</p>
          </div>
        </div>
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <AnalyticsWidget
                ...
                change={{ value: analytics.monthlyGrowth, type: "increase", period: "this month" }}
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-sm text-foreground">New student registration: Alex Johnson</span>
                      <span className="text-xs text-muted-foreground ml-auto">2m ago</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <div className="h-2 w-2 bg-blue-500 rounded-full"></div>
                      <span className="text-sm text-foreground">TechCorp posted a new job</span>
                      <span className="text-xs text-muted-foreground ml-auto">15m ago</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <div className="h-2 w-2 bg-purple-500 rounded-full"></div>
                      <span className="text-sm text-foreground">Mentor Dr. Sarah Chen approved</span>
                      <span className="text-xs text-muted-foreground ml-auto">1h ago</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <div className="h-2 w-2 bg-orange-500 rounded-full"></div>
                      <span className="text-sm text-foreground">50 new applications this week</span>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {pendingCompanies.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">No pending companies</p>
                  ) : (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {pendingMentors.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">No pending mentors</p>
                  ) : (
